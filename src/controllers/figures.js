...
const { DEFAULT_HISTOGRAM_2D, DEFAULT_HEATMAP } = require('../util/figure');
...
`, (err, data) => {
    ...
    const layout = {
      xaxis: { title: { text: 'words to choose from' }},
      yaxis: { title: { text: 'answers needed' }}
    };

    const figure = { data: [ dataTrace, lineTrace ], layout };
    res.json(figure)
  });
};
...
`, (err, data) => {
    ...
    const dataTrace = {
      ...
      // text: letters.map(letter => analysis.scrabbleScore(letter)),
      text: letters,
      type: 'scatter',
      mode: 'text',
      name: ''
    ...
    const layout = {
      xaxis: { title: { text: 'english frequency' }},
      yaxis: { title: { text: 'crossword frequency' }}
    };
...
`, (err, data) => {
    ...
    const layout = {
      xaxis: { title: { text: 'scrabble score' }},
      yaxis: { title: { text: 'crossword frequency' }}
    };
...
// 1. vowel counts at letter positions
// 2. word start heat map
// 3. vowel heat map
...
`, (err, data) => {
    ...
    const layout = {
      xaxis: { title: { text: 'letter position' }},
      yaxis: { title: { text: 'fraction vowels' }}
    };
...
AND p.width = ${PUZZLE_DIMENSION}
AND p.height = ${PUZZLE_DIMENSION}
AND (grid_index = ${gridIndex} OR grid_index = ${gridIndex * PUZZLE_DIMENSION})
GROUP BY answer
ORDER BY count DESC
LIMIT 30;
`, (err, data) => {
...
`, (err, data) => {
    ...
    const layout = {
      ...
      yaxis: { title: { text: 'unique answers' }}
    };
...
WHERE p.date BETWEEN '1000-01-01' AND '3020-01-01'
GROUP BY answer
ORDER BY date_introduced;
`, (err, data) => {
    if (err) return next(err);

    const dates = [];
    const counts = [];
    let prevDate = data.rows[0].date_introduced;
    for (let i = 1, len = data.rows.length; i < len; i++) {
      const date = data.rows[i].date_introduced;
      if (date !== prevDate) {
        dates.push(prevDate);
        counts.push(i);
        prevDate = date;
    ...
    const layout = {
      xaxis: { title: { text: 'date' }},
      yaxis: { title: { text: 'cumulative unique answers' }}
    };
...
figures.topNewWordsByYear = (req, res, next) => {
  const lifetimeUsageThreshold = cleanNumber(req.query.thresh, 2);
  const MAX_WORDS_PER_YEAR = 10;
  db.query(`
SELECT
  answer,
  MIN(DATE_TRUNC('year', p.date)) as year_introduced,
  MIN(DATE_TRUNC('day', p.date)) as day_introduced,
  COUNT(*) as occurrences
FROM clues
INNER JOIN puzzles p ON puzzle_id=p.id
WHERE p.date BETWEEN '1000-01-01' AND '3020-01-01'
GROUP BY answer
HAVING COUNT(*)>=${lifetimeUsageThreshold}
ORDER BY year_introduced DESC, occurrences DESC, day_introduced;
`, (err, data) => {
...
`, (err, data) => {
    ...
    const layout = {
      xaxis: { title: { text: 'lifespan (years)' }},
      yaxis: { title: { text: 'number answers' }}
    };
